...
class MovieQueryBuilder:
    ...
    def build_search_query(raw_query: str, limit: int = 10) -> tuple[str | None, list | None]:
        ...
        Metacharacters (%, _, and the | escape char) in the user query are
        escaped so they're treated as literal characters, not SQL pattern
        meta or escape markers.
        """
        ...
        # Using '|' (rather than '\') avoids double-interpretation under
        # MySQL's default string-literal handling of backslashes.

...
- **Runtime-created indexes**: `infra/runtime_schema.py` adds indexes at startup that aren't in the base CREATE TABLE definitions. Current list: `idx_movie_candidates_refreshed_at`, `idx_movie_candidates_shuffle` (supports the hot candidate-fetch ORDER BY at `movies/candidate_store.py:147`), and `idx_cache_filter_rand` on `popular_movies_cache` (supports filter+random queries at `movies/query_builder.py:414-415`, conditional on the ops table existing). When checking indexes manually, don't rely solely on `infra/runtime_schema.py`'s `_RUNTIME_SCHEMA_STATEMENTS` — check the `ensure_*_index` helpers too.

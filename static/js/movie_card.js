...
// Apply the dynamic backdrop image once the page has loaded.
// We read the data attribute from the backdrop container and set
// the CSS variable used in the stylesheet. This avoids embedding
// the full URL directly in the stylesheet so the template stays
// clean.
document.addEventListener("DOMContentLoaded", function () {
    const backdrop = document.querySelector('.backdrop-container');
    if (backdrop) {
        const path = backdrop.dataset.backdrop;
        if (path) {
            const fullUrl = `url('https://image.tmdb.org/t/p/original${path}')`;
            backdrop.style.setProperty('--backdrop-image', fullUrl);
        }
    }
});
